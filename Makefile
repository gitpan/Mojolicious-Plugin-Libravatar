# This Makefile is for the Mojolicious::Plugin::Libravatar extension to perl.
#
...
#     AUTHOR => [q[William Travis Holton <heytrav@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Mojolicious::Plugin::Libravatar]
#     PREREQ_PM => { Test::Routine=>q[0], Test::Exception=>q[0], Libravatar::URL=>q[0], Mojolicious=>q[3.50] }
#     VERSION_FROM => q[lib/Mojolicious/Plugin/Libravatar.pm]
#     test => { TESTS=>q[t/*.t] }
...
EXE_EXT = 
FULL_AR = /usr/bin/ar
LD = env MACOSX_DEPLOYMENT_TARGET=10.7 /usr/bin/clang
...
MAN3EXT = 3pm
INSTALLDIRS = site
INSTALL_BASE = /Users/holton/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
PERL_LIB = /opt/local/lib/perl5/5.16.1
...
PERL = /opt/local/bin/perl5.16
FULLPERL = /opt/local/bin/perl5.16
ABSPERL = $(PERL)
PERLRUN = $(PERL)
...
MAKE = make
FULLEXT = Mojolicious/Plugin/Libravatar
BASEEXT = Libravatar
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Mojolicious/Plugin/Libravatar.pm
...
TO_INST_PM = lib/Mojolicious/Plugin/Libravatar.pm

PM_TO_BLIB = lib/Mojolicious/Plugin/Libravatar.pm \
...
manifypods : pure_all  \
	lib/Mojolicious/Plugin/Libravatar.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Mojolicious/Plugin/Libravatar.pm $(INST_MAN3DIR)/Mojolicious::Plugin::Libravatar.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''William Travis Holton <heytrav@cpan.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Mojolicious-Plugin-Libravatar' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  Libravatar::URL: 0' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "William Travis Holton <heytrav@cpan.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Mojolicious-Plugin-Libravatar",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "Libravatar::URL" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Mojolicious" : "3.50",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Test::Exception" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Test::Routine" : "0"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>William Travis Holton &lt;heytrav@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Libravatar::URL" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Mojolicious::" VERSION="3.5" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Exception" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Routine" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Mojolicious/Plugin/Libravatar.pm blib/lib/Mojolicious/Plugin/Libravatar.pm 
